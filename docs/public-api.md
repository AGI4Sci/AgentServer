...
For production-like Claude Code/self-hosted checks, preflight and runtime share the same model runtime resolver. Configure the shared endpoint once in `openteam.json` (`llm.baseUrl`, `llm.apiKey`, `llm.model`, `llm.provider`); AgentServer maps that JSON into the canonical runtime env it needs. `AGENT_SERVER_MODEL_*` and older `AGENT_SERVER_ADAPTER_LLM_*` names remain compatibility overrides for one-off tests, but they should not become a second persistent configuration source.
...
AgentServer answers Codex app-server approval requests automatically by default: command and file-change approvals use `acceptForSession`, while permissions requests grant the requested permission subset with session scope. This keeps unattended backend smoke and AgentServer-driven runs moving without waiting for a UI approval roundtrip.

...
本文档描述 **自研/custom agent backend 的内部 harness context 策略**。

它不是 AgentServer Core 的公共接口，也不是所有 backend 必须遵守的协议。AgentServer Core 的通用 context 契约见 [Core Context Contract](./context-core.md)。

推荐边界：

```text
context-core.md
  AgentServer Core 通用、稳定、跨 backend 的 external auditable context 契约

context-harness.md
  v9/custom backend harness 内部可实验的 prefix/work、stable/dynamic、COMPACTION TAG 策略
```

...
AgentServer Core
  ...
  不内置复杂自进化决策
        ...
        v
Backend Harness
  claude-code / codex / openclaw / zeroclaw / v9
  保持自治
...
Backend Runtime
  ...
  Runtime event normalization
        ...
        v
Agent Backend
  claude-code / codex / openclaw / zeroclaw / v9 / future backend
...
它们的语义不是“裸 runtime run”，而是长期 agent run：
...
但底层语义仍然应该保持：AgentServer 负责长期状态、上下文、恢复和审计。
...
- `backend` 指向具体 backend，如 `claude-code`、`codex`、`openclaw`、`zeroclaw`、`v9`

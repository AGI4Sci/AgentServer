...
- `T035`：Agent Backend Orchestration 最终架构（推进中）：AgentServer 从多 backend facade 升级为多 agent backend 编排层，对外仍表现为一个连续 agent；架构文档已更新，后续进入接口/实现设计；最近更新 `2026-04-19`
- `T036`：Codex / Claude Code 完整 agent-backend adapter（待推进）：将 Codex app-server 和 Claude Code 作为完整 agent backend 接入，而不是只作为普通 model provider；最近更新 `2026-04-19`
- `T037`：统一上下文与 backend handoff 契约（待推进）：由 AgentServer 持有 canonical session context，各 backend 只接收面向任务阶段的 handoff packet；最近更新 `2026-04-19`
- `T038`：Live Backend Benchmark 独立模块（待讨论，暂不实现）：记录需要独立设计 backend 评估与路由打分模块，后续单独深入讨论；最近更新 `2026-04-19`
- `T039`：Run/Stage 状态机与 Adapter Contract 文档（待推进）：补齐 Stage 一等模型、状态机、adapter contract、metadata 治理和 evolution risk checker；最近更新 `2026-04-19`
- `T040`：Native Session Scope 与 Orchestrator Policy 边界（已记录）：明确 backend native session 默认 session-scoped 复用但不是真相源，orchestrator 拆为 core kernel 与可插拔 policy；最近更新 `2026-04-19`
...
- 对外仍暴露一个连续 agent/session/run；内部可以按任务阶段调用不同 backend，例如 Codex 负责审查和找 bug，Claude Code 负责实现，Gemini/其它 backend 负责长上下文、多模态或规划。
...
#### 目标说明
- 将 Codex app-server / SDK 和 Claude Code 接为完整 `agent_backend`，而不是继续只通过普通 provider 抽象调用模型。
- 让代码任务可以在一次 request 内复用 backend 的完整能力：agent loop、官方工具事件、thread/session、approval、sandbox 或等价执行策略。
- 保留 `openai-codex` direct provider 的定位为轻量/兼容/兜底路径；强代码任务优先走 Codex app-server 或 Claude Code agent backend。

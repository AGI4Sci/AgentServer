...
export class OpenClawSessionClient implements SessionRunner {
  async run(input: SessionInput, options: RunSessionOptions): Promise<SessionOutput> {
    ...
    return finalOutput || { success: false, error: 'OpenClaw runtime exited without result event.' };
  }
  ...
  async runStream(
    ...
    return await runSessionViaSupervisor('openclaw', input, options, handlers);
  }
}

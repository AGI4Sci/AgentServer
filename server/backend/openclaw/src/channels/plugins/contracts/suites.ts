...
async function sendSlackPayload(params: {
  to: string;
  payload: ReplyPayload;
  sendSlack: OutboundSendMock;
}): Promise<Record<string, unknown>> {
  const sendResult = async (text: string, mediaUrl?: string) =>
    await params.sendSlack(params.to, text, mediaUrl ? { mediaUrl } : {});

  if (typeof params.payload.mediaUrl === "string" && params.payload.mediaUrl.trim()) {
    return { channel: "slack", ...(await sendResult(params.payload.text ?? "", params.payload.mediaUrl)) };
  }

  if (Array.isArray(params.payload.mediaUrls) && params.payload.mediaUrls.length > 0) {
    let lastResult: Record<string, unknown> = { messageId: "" };
    for (const [index, mediaUrl] of params.payload.mediaUrls.entries()) {
      if (typeof mediaUrl !== "string" || !mediaUrl.trim()) {
        continue;
      }
      lastResult = await sendResult(index === 0 ? (params.payload.text ?? "") : "", mediaUrl);
    }
  ...
  if (text) {
    return { channel: "slack", ...(await sendResult(params.payload.text ?? "")) };
  }

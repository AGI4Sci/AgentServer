...
struct ConnectParams {
    ...
    /// Optional provider override for this websocket session.
    #[serde(default)]
    model_provider: Option<String>,
    /// Optional model override for this websocket session.
    #[serde(default)]
...
async fn handle_socket(
    ...
    let persisted_messages = if let Some(ref backend) = state.session_backend {
                ...
                if let Ok(cp) = serde_json::from_str::<ConnectParams>(&text) {
                    if cp.msg_type == "connect" {
                        debug!(
                            ...
                            model_provider = ?cp.model_provider,
                            model_name = ?cp.model_name,
                            "WebSocket connect params received"
                        );
                        connect_params = Some(cp);
                    } else {
    ...
    let mut effective_config = state.config.lock().clone();
    if let Some(cp) = &connect_params {
        if let Some(provider) = cp
            .model_provider
            .as_ref()
        ...
        if let Some(model_name) = cp
            .model_name
            .as_ref()
            .map(|value| value.trim())
            .filter(|value| !value.is_empty())
        {
            effective_config.default_model = Some(model_name.to_string());
        }
    }

    let provider_label = effective_config
        .default_provider
        .clone()
        .unwrap_or_else(|| "unknown".to_string());
    let model_label = effective_config
        .default_model
        .clone()
        .unwrap_or_else(|| "(default)".to_string());

    let mut agent = match crate::agent::Agent::from_config(&effective_config).await {
        Ok(a) => a,
        Err(e) => {

 ...
 */
interface OnStartEvent<TOOLS extends ToolSet = ToolSet, RUNTIME_CONTEXT extends Context = Context, OUTPUT extends Output = Output> {
    /** Unique identifier for this generation call, used to correlate events. */
    readonly callId: string;
 ...
 */
interface OnStepStartEvent<TOOLS extends ToolSet = ToolSet, RUNTIME_CONTEXT extends Context = Context, OUTPUT extends Output = Output> {
    /** Unique identifier for this generation call, used to correlate events. */
    readonly callId: string;
...
interface Telemetry {
     ...
     */
    onStart?: Callback<OnStartEvent | ObjectOnStartEvent | EmbedOnStartEvent | RerankOnStartEvent>;
    /**
     ...
     */
    onFinish?: Callback<OnFinishEvent<ToolSet> | ObjectOnFinishEvent<unknown> | EmbedOnFinishEvent | RerankOnFinishEvent>;
    /**
 ...
 */
type ToolNeedsApprovalConfiguration<TOOLS extends ToolSet> = {
    [key in keyof TOOLS]?: boolean | ToolNeedsApprovalFunction<InferToolInput<TOOLS[key]>, InferToolContext<TOOLS[key]>>;
};
 ...
 */
type StreamTextOnStepFinishCallback<TOOLS extends ToolSet, RUNTIME_CONTEXT extends Context> = Callback<OnStepFinishEvent<TOOLS, RUNTIME_CONTEXT>>;
/**
 ...
 * @param event - The event that is passed to the callback.
 */
type StreamTextOnFinishCallback<TOOLS extends ToolSet, RUNTIME_CONTEXT extends Context> = Callback<OnFinishEvent<TOOLS, RUNTIME_CONTEXT>>;
/**
 ...
 */
type StreamTextOnStartCallback<TOOLS extends ToolSet = ToolSet, RUNTIME_CONTEXT extends Context = Context, OUTPUT extends Output = Output> = Callback<OnStartEvent<TOOLS, RUNTIME_CONTEXT, OUTPUT>>;
/**
...
declare function streamText<TOOLS extends ToolSet, RUNTIME_CONTEXT extends Context = Context, OUTPUT extends Output = Output<string, string, never>>({ model, tools, toolChoice, system, prompt, messages, maxRetries, abortSignal, timeout, headers, stopWhen, output, toolNeedsApproval, experimental_telemetry, telemetry, prepareStep, providerOptions, activeTools, experimental_repairToolCall: repairToolCall, experimental_transform: transform, experimental_download: download, includeRawChunks, onChunk, onError, onFinish, onAbort, onStepFinish, experimental_onStart: onStart, experimental_onStepStart: onStepStart, experimental_onToolExecutionStart: onToolExecutionStart, experimental_onToolExecutionEnd: onToolExecutionEnd, runtimeContext, toolsContext, experimental_include: include, _internal: { now, generateId, generateCallId, }, ...settings }: LanguageModelCallOptions & RequestOptions<TOOLS> & Prompt & ToolsContextParameter<TOOLS> & {
     ...
     */
    onFinish?: StreamTextOnFinishCallback<NoInfer<TOOLS>, NoInfer<RUNTIME_CONTEXT>>;
    onAbort?: StreamTextOnAbortCallback<NoInfer<TOOLS>, NoInfer<RUNTIME_CONTEXT>>;
    /**
     * Callback that is called when each step (LLM call) is finished, including intermediate steps.
     */
    onStepFinish?: StreamTextOnStepFinishCallback<NoInfer<TOOLS>, NoInfer<RUNTIME_CONTEXT>>;
    /**
     * Callback that is called when the streamText operation begins,
     * before any LLM calls are made.
     */
    experimental_onStart?: StreamTextOnStartCallback<NoInfer<TOOLS>, NoInfer<RUNTIME_CONTEXT>, NoInfer<OUTPUT>>;
    /**
     * Callback that is called when a step (LLM call) begins,
     * before the provider is called.
     */
    experimental_onStepStart?: StreamTextOnStepStartCallback<NoInfer<TOOLS>, NoInfer<RUNTIME_CONTEXT>, NoInfer<OUTPUT>>;
    /**
...
type ToolLoopAgentSettings<CALL_OPTIONS = never, TOOLS extends ToolSet = {}, RUNTIME_CONTEXT extends Context = Context, OUTPUT extends Output = never> = LanguageModelCallOptions & Omit<RequestOptions<TOOLS>, 'abortSignal'> & ToolsContextParameter<TOOLS> & {
     ...
     */
    experimental_onStart?: ToolLoopAgentOnStartCallback<NoInfer<TOOLS>, RUNTIME_CONTEXT, NoInfer<OUTPUT>>;
    /**
     * Callback that is called when a step (LLM call) begins, before the provider is called.
     */
    experimental_onStepStart?: ToolLoopAgentOnStepStartCallback<NoInfer<TOOLS>, NoInfer<RUNTIME_CONTEXT>, NoInfer<OUTPUT>>;
    /**
     ...
     * Callback that is called when each step (LLM call) is finished, including intermediate steps.
     */
    onStepFinish?: ToolLoopAgentOnStepFinishCallback<NoInfer<TOOLS>, NoInfer<RUNTIME_CONTEXT>>;
    /**
     * Callback that is called when all steps are finished and the response is complete.
     */
    onFinish?: ToolLoopAgentOnFinishCallback<NoInfer<TOOLS>, NoInfer<RUNTIME_CONTEXT>>;
    /**
     * Additional provider-specific options. They are passed through
     * to the provider from the AI SDK and enable provider-specific
     * functionality that can be fully encapsulated in the provider.
...
type AgentCallParameters<CALL_OPTIONS, TOOLS extends ToolSet = {}, RUNTIME_CONTEXT extends Context = Context> = ([CALL_OPTIONS] extends [never] ? {
     ...
     * Callback that is called when a step (LLM call) begins, before the provider is called.
     */
    experimental_onStepStart?: ToolLoopAgentOnStepStartCallback<TOOLS, RUNTIME_CONTEXT>;
    /**
     * Callback that is called before each tool execution begins.
     */
 ...
 * @param event - The event object containing generation configuration.
 */
type GenerateTextOnStartCallback<TOOLS extends ToolSet = ToolSet, RUNTIME_CONTEXT extends Context = Context, OUTPUT extends Output = Output> = Callback<OnStartEvent<TOOLS, RUNTIME_CONTEXT, OUTPUT>>;
/**
 * Callback that is set using the `experimental_onStepStart` option.
 *
 * Called when a step (LLM call) begins, before the provider is called.
 ...
 * @param stepResult - The result of the step.
 */
type GenerateTextOnStepFinishCallback<TOOLS extends ToolSet = ToolSet, RUNTIME_CONTEXT extends Context = Context> = Callback<OnStepFinishEvent<TOOLS, RUNTIME_CONTEXT>>;
/**
 * Callback that is set using the `onFinish` option.
 *
 * Called when the entire generation completes (all steps finished).
 ...
 */
type GenerateTextOnFinishCallback<TOOLS extends ToolSet = ToolSet, RUNTIME_CONTEXT extends Context = Context> = Callback<OnFinishEvent<TOOLS, RUNTIME_CONTEXT>>;
/**
 * Generate a text and call tools for a given prompt using a language model.
 *
...
declare function generateText<TOOLS extends ToolSet, RUNTIME_CONTEXT extends Context = Context, OUTPUT extends Output = Output<string, string>>({ model: modelArg, tools, toolChoice, system, prompt, messages, maxRetries: maxRetriesArg, abortSignal, timeout, headers, stopWhen, output, toolNeedsApproval, experimental_telemetry, telemetry, providerOptions, activeTools, prepareStep, experimental_repairToolCall: repairToolCall, experimental_download: download, runtimeContext, toolsContext, experimental_include: include, _internal: { generateId, generateCallId, }, experimental_onStart: onStart, experimental_onStepStart: onStepStart, experimental_onToolExecutionStart: onToolExecutionStart, experimental_onToolExecutionEnd: onToolExecutionEnd, onStepFinish, onFinish, ...settings }: LanguageModelCallOptions & RequestOptions<TOOLS> & Prompt & ToolsContextParameter<TOOLS> & {
     ...
     * Callback that is called when all steps are finished and the response is complete.
     */
    onFinish?: GenerateTextOnFinishCallback<NoInfer<TOOLS>, NoInfer<RUNTIME_CONTEXT>>;
    /**
     * Settings for controlling what data is included in step results.
     * Disabling inclusion can help reduce memory usage when processing
     * large payloads like images.
 ...
 */
declare function streamLanguageModelCall<TOOLS extends ToolSet, OUTPUT extends Output = Output>({ model, tools, output, toolChoice, prompt, system, messages, download, abortSignal, headers, includeRawChunks, providerOptions, repairToolCall, onStart, ...callSettings }: {
    model: LanguageModel;
    tools?: TOOLS;
    output?: OUTPUT;
    toolChoice?: ToolChoice<TOOLS>;
 ...
 */
declare function createAgentUIStream<CALL_OPTIONS = never, TOOLS extends ToolSet = {}, RUNTIME_CONTEXT extends Context = Context, OUTPUT extends Output = never, MESSAGE_METADATA = unknown>({ agent, uiMessages, options, abortSignal, timeout, experimental_transform, onStepFinish, ...uiMessageStreamOptions }: {
    agent: Agent<CALL_OPTIONS, TOOLS, RUNTIME_CONTEXT, OUTPUT>;
    uiMessages: unknown[];
    abortSignal?: AbortSignal;
    timeout?: TimeoutConfiguration<TOOLS>;
 ...
 */
declare function embed({ model: modelArg, value, providerOptions, maxRetries: maxRetriesArg, abortSignal, headers, experimental_telemetry, telemetry, experimental_onStart: onStart, experimental_onFinish: onFinish, _internal: { generateCallId }, }: {
    /**
 ...
 */
declare function embedMany({ model: modelArg, values, maxParallelCalls, maxRetries: maxRetriesArg, abortSignal, headers, providerOptions, experimental_telemetry, telemetry, experimental_onStart: onStart, experimental_onFinish: onFinish, _internal: { generateCallId }, }: {
    /**
     * The embedding model to use.
     */
    model: EmbeddingModel;

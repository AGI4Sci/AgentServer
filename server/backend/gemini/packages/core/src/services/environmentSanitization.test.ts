...
describe('sanitizeEnvironment', () => {
  ...
  it('should redact variables with values matching all token and credential patterns', () => {
    const env = {
      // GitHub
      GITHUB_TOKEN_GHP: 'github_token_ghp_redacted',
      GITHUB_TOKEN_GHO: 'github_token_gho_redacted',
      GITHUB_TOKEN_GHU: 'github_token_ghu_redacted',
      GITHUB_TOKEN_GHS: 'github_token_ghs_redacted',
      GITHUB_TOKEN_GHR: 'github_token_ghr_redacted',
      GITHUB_PAT: 'github_pat_redacted',
      // Google
      GOOGLE_KEY: 'google_api_key_redacted',
      // AWS
      AWS_KEY: 'aws_access_key_redacted',
      // JWT
      JWT_TOKEN: 'jwt_token_redacted',
      // Stripe
      STRIPE_SK_LIVE: 'stripe_live_secret_key_redacted',
      STRIPE_RK_LIVE: 'stripe_live_restricted_key_redacted',
      STRIPE_SK_TEST: 'stripe_test_secret_key_redacted',
      STRIPE_RK_TEST: 'stripe_test_restricted_key_redacted',
      // Slack
      SLACK_XOXB: 'slack_bot_token_redacted',
      SLACK_XOXA: 'slack_app_token_redacted',
      SLACK_XOXP: 'slack_user_token_redacted',
      SLACK_XOXB_2: 'slack_refresh_token_redacted',
      // URL Credentials
  ...
  describe('value-first security: secret values must be caught even for allowed variable names', () => {
    ...
    it('should NOT redact GEMINI_CLI_ variables even if their value looks like a secret (fully trusted)', () => {
      const env = {
        GEMINI_CLI_INTERNAL: `ghp_${'x'.repeat(36)}`,
      };
      const sanitized = sanitizeEnvironment(env, EMPTY_OPTIONS);
      expect(sanitized).toEqual(env);
    });
  ...
  it('should not perform any redaction if enableEnvironmentVariableRedaction is false', () => {
    const env = {
      ...
      db_password: 'password-value',
      RSA_KEY: '-----BEGIN RSA PRIVATE KEY-----...',
      GITHUB_TOKEN_GHP: `ghp_${'x'.repeat(36)}`,
      SAFE_VAR: 'is-safe',
    };
    const options = {
      allowedEnvironmentVariables: [],

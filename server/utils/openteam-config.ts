...
export function applyOpenTeamConfigEnv(options?: { overwrite?: boolean }): OpenTeamConfig {
  ...
  const mappings: Array<[string, string | null | undefined]> = [
    ...
    ['AGENT_SERVER_MODEL_BASE_URL', config.llm.baseUrl],
    ['AGENT_SERVER_MODEL_API_KEY', config.llm.apiKey],
    ['AGENT_SERVER_MODEL_NAME', config.llm.model],
    ['AGENT_SERVER_MODEL_PROVIDER', config.llm.provider],
